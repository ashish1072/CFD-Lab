...
int main(int argn, char** args){
    ...
    /* WS2 new variables */
    double **T = NULL;
    double *TI = malloc(sizeof(double*));
    double *T_h = malloc(sizeof(double*));
    double *T_c = malloc(sizeof(double*));
    double *PR =  malloc(sizeof(double*));
    double *beta = malloc(sizeof(double*));
    
    

    read_parameters(file, Re, UI, VI, PI, GX, GY, t_end, xlength, ylength, dt, dx, dy, imax, 
                    jmax, alpha, omg, tau, itermax, eps, dt_value, TI, T_h, T_c, PR, beta);
    ...
    init_uvp(*UI, *VI, *PI, *imax, *jmax, U, V, P, T, *TI);
    ...
    while( t < *t_end)
    {
      ...
      boundaryvalues(*imax, *jmax, U, V, T, *T_h, *T_c);
      calculate_temp(*Re, *alpha, *dt, *dx, *dy, *imax, *jmax, U, V, T, *PR);
      calculate_fg(*Re, *GX, *GY, *alpha, *dt, *dx, *dy, *imax, *jmax, U, V, F, G, T, *beta);
      ...
      *res = 50;  // random value of residual that is greater than eps
      ...
      printf("t = %lf, dt = %lf, SOR Iterations = %d \n", t, *dt, it);

      if(t >= x*(*dt_value)){
	      write_vtkFile("szProblem", n, *xlength, *ylength, *imax, *jmax, *dx, *dy, U, V, P, T);
        x =x+1;} 
    ...
    free_matrix( T, 0, (*imax) + 2, 0, (*jmax) + 2 );
